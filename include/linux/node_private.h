...
/**
 * node_is_private - is @nid a private (N_MEMORY_PRIVATE) node?
 * @nid: the node to test
 *
 * NUMA=n-safe wrapper for the N_MEMORY_PRIVATE node state.  With
 * MAX_NUMNODES == 1, node_state() returns true for node 0 in *every* state
 * (it collapses to "node == 0"), which would spuriously classify the only
 * node of a UMA system as private.  Route all N_MEMORY_PRIVATE tests through
 * here so they read false when private nodes cannot exist.
 */
