...
#else
static inline bool node_device_allows_longterm_pin(int nid) { return false; }
#endif
...
static inline bool folio_is_longterm_pinnable(struct folio *folio)
{
	...
	/*
	 * Managed device node folios are not longterm pinnable by default.
	 * Services that support pinning opt in via PGMAP_OPS_LONGTERM_PIN.
	 * node_device_allows_longterm_pin() is out-of-line because
	 * node_device.h includes mm.h (circular dependency).
	 *
	...
	if (node_is_private(folio_nid(folio)))
		return node_device_allows_longterm_pin(folio_nid(folio));
#endif
