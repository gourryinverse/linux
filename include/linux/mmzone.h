 ...
 * @owner: opaque driver identity; owns the memory and authorises cap updates
 * @caps: NODE_MEMORY_CAP_* services the driver opts the node into
 *
 * Allocated and owned by the registering driver.  Defined here alongside
 * pg_data_t so consumers that reach it through pg_data_t.node_private see a
 * complete type.
 */
struct node_private {
	void *owner;
