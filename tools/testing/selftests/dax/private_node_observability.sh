...
#
# Private-node observability: user-visible accounting includes private nodes.
#
#   1. memcg memory.numa_stat reports private-node anon.
...
#
# Needs a kmem-private dax device on a memoryless node; SKIPs otherwise, and each
# check SKIPs when its prereq (cgroup2 memory, THP, debugfs, python3) is absent.
