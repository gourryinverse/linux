...
static int t_shmem_excluded(void)
{
	...
	/*
	 * on_cram == 0 proves nothing unless the folios actually left memory.
	 * A MADV_PAGEOUT that silently did nothing satisfies it trivially, so
	 * report that as environmental rather than as a pass.
	 */
	if (oncram)
		return 1;	/* shmem is not cram-eligible: any page is a leak */
	return (before - after < before / 2) ? 3 : 0;
}
