...
# ---------------------------------------------------------------------------
# 1. CONTAINMENT: a node that opted into nothing relevant, online_kernel (the
#    most permissive zone).
#
#    Containment is only contracted for services the node did NOT opt into.  A
#    HUGETLB node is *supposed* to satisfy a per-node pool write, and a DEMOTION
#    node is a legitimate reclaim target, so asserting free==total on either
#    would be asserting that an opt-in does not work.  Features are fixed when a
#    node is provisioned, so pick a node that took neither rather than trying to
#    clear them.  This cell keeps its own node; $PN is left for cells 2 and 3.
# ---------------------------------------------------------------------------
CN=
for nid in $(pn_private_nodes); do
	pn_node_has_feature "$nid" "$(pn__feat_mask user_numa)" && continue
	pn_node_has_feature "$nid" "$(pn__feat_mask demotion)" && continue
	CN=$nid; break
done

if [ -z "$CN" ]; then
	ktap_test_skip "no private node without HUGETLB and DEMOTION to contain"
elif ! pn_online_as online; then
	ktap_test_skip "could not online node $PN as private (state=$(pn_state))"
else
	ktap_print_msg "containment node $CN (features $(pn_node_features "$CN"))"
	tot=$(node_total_kb "$CN")
	free0=$(node_free_kb "$CN")
	spill=0 why=
	[ "${tot:-0}" -gt 0 ] || { spill=1; why="$why no-memory;"; }

	# Containment means nothing NEW lands on the node, which is not the same
	# as the node being empty: every test shares one guest, so an earlier one
	# may still be draining pages here.  Watch for free going DOWN, not for
	# free == total, or a dirty starting point reads as a breach.
	pn_contained() {	# $1 = what was just attempted
		local now; now=$(node_free_kb "$CN")
		[ "$now" -ge "$free0" ] 2>/dev/null && return 0
		spill=1; why="$why $1(free=$now start=$free0);"
	}

	# per-node hugetlb pool write: an explicit __GFP_THISNODE alloc must place 0.
	hp="$NODE_BASE/node$CN/hugepages/hugepages-2048kB/nr_hugepages"
	if [ -w "$hp" ]; then
		hp0=$(cat "$hp" 2>/dev/null); pn_save_global "$hp"
		echo 64 > "$hp" 2>/dev/null
		[ "$(cat "$hp")" = 0 ] || { spill=1; why="$why hugetlb=$(cat "$hp");"; }
		pn_contained hugetlb
	...
	for _ in 1 2 3 4 5 6; do
		sleep 3
		pn_contained pressure
	done
	kill "$ch" 2>/dev/null; wait "$ch" 2>/dev/null

	if [ "$spill" = 0 ]; then
		ktap_test_pass "private node $CN took nothing under node-0 pressure + THISNODE hugetlb (free $free0 -> $(node_free_kb "$CN") kB)"
	else
		ktap_test_fail "containment breach on node $CN:$why"
	fi
fi
pn_reset
...
if ! pn_online_as online; then
	...
	if [ "${man_mb:-0}" -lt 64 ]; then
		...
		"$TOOL" daxmap "$PN_ANON" "$hog_mb" "$PN" 120 >/dev/null 2>&1 &
		hp=$!
		sleep 4
		"$TOOL" daxmap "$PN_ANON" "$trig_mb" "$PN" 0 >/dev/null 2>&1 &
		tp=$!
		wait "$tp"; trc=$?
