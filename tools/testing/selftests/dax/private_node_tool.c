 ...
 * did not opt into NODE_MEMORY_FEAT_LTPIN must fail the pin AND stay in place (it
 ...
 * (FEAT_RECLAIM), and the collapsed THP should sit on @nid.
 */
 ...
 * (FEAT_USER_NUMA) -- report the rc only and immediately reset to the default
 ...
 * ({base} here) -- so even a private home node must not be feature-gated,
 * and must not pull the allocation onto itself.  Report rc, and on success
 * fault and report how many pages landed on the home node (expected 0 when
 * home != base).
 */
 ...
 * its node allows mempolicy placement (FEAT_USER_NUMA) -- otherwise -ENOENT.
 */
 ...
 * move_pages() target only when it allows mempolicy placement (FEAT_USER_NUMA);
 ...
 * new={newn}) and report residency.  Probes the FEAT_USER_NUMA gate on both ends:
