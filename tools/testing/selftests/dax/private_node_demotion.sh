...
#   2. Demote FROM private: a 2nd private node placed ABOVE DRAM demotes its
#      overflow into DRAM.
#   3. Without the demotion feature, a private node is not a demotion target.
#
...
# Tier position is declared by the provider (dax_test adistance=) or derived
# from HMAT; a test can no longer place a node in a tier itself.  Require the
# ordering the assertions below depend on and skip if the topology lacks it.
tier_of() {	# tier_of NID -- the memory_tierN a node belongs to, if any
	local t
	for t in /sys/devices/virtual/memory_tiering/memory_tier*; do
		[ -r "$t/nodelist" ] || continue
		pn_nodelist_has "$t/nodelist" "$1" && { basename "$t"; return; }
	done
}
[ -n "$(tier_of "$P1")" ] || {
	ktap_skip_all "node $P1 is in no memory tier: provision it with dax_test adistance=2880, or supply HMAT"
	exit "$KSFT_SKIP"; }
...
# 2. demote FROM private (node2, above DRAM) ----------------------------------
if [ -z "$P2" ]; then
...
# 3. NEGATIVE: demotion cleared -> private node is NOT a demotion target ----
...
pn_reset; pn_set reclaim 1; pn_set demotion 0
