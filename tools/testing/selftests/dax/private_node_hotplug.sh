...
# 2. a bogus capability value is rejected, mask unchanged
before=$(cat "$CAPF" 2>/dev/null)
echo notanumber > "$CAPF" 2>/dev/null; rc=$?
if [ "$rc" != 0 ] && [ "$(cat "$CAPF" 2>/dev/null)" = "$before" ]; then
	ktap_test_pass "invalid capability value rejected (unchanged)"
else
	ktap_test_fail "invalid capability value not rejected: rc=$rc caps=$(cat "$CAPF" 2>/dev/null)"
fi
