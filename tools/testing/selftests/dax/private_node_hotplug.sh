...
#
# Private-node feature + hotplug ABI test.
#
...
#
# Features are recorded while unplugged; dependencies (demotion needs reclaim)
# are enforced once at hotplug by node_memory_features_register(), not by the
# setters.
...
if [ "$rc" != 0 ] && [ "$(pn_state)" = unplugged ] && ! pn_node_online; then
	...
	ktap_test_fail "inconsistent features plugged in: rc=$rc state=$(pn_state) online=$(pn_node_online && echo 1 || echo 0)"
fi
...
if [ "$rc" = 0 ] && [ "$(pn_state)" = online_kernel ] && pn_online_private; then
	...
	ktap_test_fail "consistent features failed to plug in: rc=$rc state=$(pn_state) online=$(pn_node_online && echo 1 || echo 0)"
fi
