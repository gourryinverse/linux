...
TOOL="$DIR/cram_readahead_tool"
FS=${CRAM_FS:-ext4}
DISK=${CRAM_FS_DISK:-/dev/vda}
MNT=/tmp/cramra	# guest root is read-only; /tmp is a writable tmpfs
...
dmesg -C 2>/dev/null
"$TOOL" "$PN" "$MNT" 2>&1 | tee /tmp/cram_readahead.out
rc=${PIPESTATUS[0]}
out=$(cat /tmp/cram_readahead.out)
splat=$(dmesg 2>/dev/null | grep -ciE "KASAN|BUG:|use-after-free|general protection|Oops|refcount_t|WARNING:")

# 1. sequential read serves resident folios in place; splice promotes
if [ "$rc" = 0 ]; then
	ktap_test_pass "readahead: sequential read serves CRAM in place, splice promotes on $FS [$out]"
elif [ "$rc" = 3 ]; then
	ktap_test_skip "clean file folios did not go resident on CRAM (environmental): $out"
else
	ktap_test_fail "readahead serve-in-place contract failed (rc=$rc): $out"
fi

# 2. no kernel splat during the read/splice round trip
if [ "$splat" = 0 ]; then
	ktap_test_pass "no KASAN/BUG/UAF/WARN during readahead round trip"
else
	ktap_test_fail "kernel splat during readahead round trip (count=$splat)"
fi
