...
#      as a public N_MEMORY node where mbind needs NO feature; setting it again
...
# public phase: ordinary mbind to P, no feature involved
out=$("$TOOL" mbind "$PN" 8 2>&1); echo "$out" | sed 's/^/# /'
placed=$(echo "$out" | pn_field "on_node$PN")
pub_ok=0
...
# The public phase is the key contract (mbind with NO feature); report the return-to-
# private leg too.
if [ "$pub_ok" = 1 ]; then
	if [ "$priv_ok" = 1 ]; then
		ktap_test_pass "E5 P private->public(private=0; mbind needs NO feature)->private, full cycle clean"
	else
		ktap_test_pass "E5 P->public(private=0): public node took mbind with NO feature (return-to-private did not re-establish in-guest; informational)"
	fi
else
