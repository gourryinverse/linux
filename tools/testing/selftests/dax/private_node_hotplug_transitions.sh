...
#      has_private_memory, the task survives.
#   E2 re-plug P private: a fresh mbind({P}) is honored again.
#   E3 hotunplug=0: a migration-driven unplug of a referenced node is refused -
#      the node stays online and its pages stay put (CAP_HOTUNPLUG negative).
...
up_private() {	# online PN as private with user_numa+hotunplug opt-ins
	pn_reset
	pn_set private 1
	pn_set user_numa 1
	pn_set hotunplug 1
...
ktap_print_msg "E1 onP:$onP0->$onP1 unplug_rc=$rc state=$(pn_state) private=$(pn_is_private && echo 1 || echo 0) alive=$alive"
if [ "$rc" = 0 ] && [ "$(pn_state)" = unplugged ] && ! pn_is_private && [ "$alive" = 1 ] && [ "${onP1:-1}" = 0 ]; then
	ktap_test_pass "E1 unplug migrated $onP0 pages off $PN and the task survived"
else
	ktap_test_fail "E1 unplug-while-bound failed (rc=$rc onP:$onP0->$onP1 alive=$alive private=$(pn_is_private && echo 1 || echo 0))"
fi
...
# --- E3: hotunplug=0 -> a migration-driven unplug is refused, pages stay -------
up_hu0() { pn_reset; pn_set private 1; pn_set user_numa 1; pn_set hotunplug 0; pn_hotplug online_movable; }
up_hu0
if [ "$(pn_state)" = online_movable ] && pn_is_private; then
	: > "$HF"
	"$TOOL" mbindhold bind "$PN" 16 40 >"$HF" 2>&1 &
	...
	pn_hotplug unplugged; rc=$?
	onP1=$(nm_on_node "$HPID" "$HADDR" "$PN")
	ktap_print_msg "E3 hotunplug=0 onP:$onP0->$onP1 unplug_rc=$rc state=$(pn_state)"
	if [ "$rc" != 0 ] && [ "$(pn_state)" = online_movable ] && pn_is_private && [ "${onP1:-0}" -gt 0 ] 2>/dev/null; then
		ktap_test_pass "E3 unplug refused without CAP_HOTUNPLUG; pages stayed on $PN"
	else
		ktap_test_fail "E3 unplug not refused with hotunplug=0 (rc=$rc state=$(pn_state) onP:$onP0->$onP1)"
	fi
	kill -9 "$HJOB" 2>/dev/null; wait "$HJOB" 2>/dev/null
else
	ktap_test_skip "E3 could not online $PN private with hotunplug=0"
fi
...
pn_hotplug online_movable
sleep 1
ktap_print_msg "E5 after private=0: has_memory=$(cat "$NODE_BASE/has_memory") has_private=$(cat "$NODE_BASE/has_private_memory" 2>/dev/null)"
...
ktap_print_msg "E5 public_phase_ok=$pub_ok back_to_private_ok=$priv_ok (private=$(pn_is_private && echo 1 || echo 0))"
