...
TOOL="$DIR/cram_pgcache_large_tool"
FS=${CRAM_FS:-ext4}
DISK=${CRAM_FS_DISK:-/dev/vda}
MNT=/tmp/cramlg	# guest root is read-only; /tmp is a writable tmpfs
...
trap 'umount "$MNT" 2>/dev/null; sync; echo 3 > /proc/sys/vm/drop_caches 2>/dev/null; echo unplugged > "$D/state" 2>/dev/null' EXIT

ktap_set_plan 2

dmesg -C 2>/dev/null
"$TOOL" "$PN" "$MNT" 2>&1 | tee /tmp/cram_pgcache_large.out
rc=${PIPESTATUS[0]}
out=$(cat /tmp/cram_pgcache_large.out)
splat=$(dmesg 2>/dev/null | grep -ciE "KASAN|BUG:|use-after-free|general protection|Oops|refcount_t|WARNING:")

# 1. large (PMD) folio resident read-in-place + shared-write promote-off-cram
if [ "$rc" = 0 ]; then
	ktap_test_pass "large file tier: PMD folio resident, read in place, write promotes off CRAM on $FS [$out]"
elif [ "$rc" = 3 ]; then
	ktap_test_skip "no PMD THP formed / not resident (environmental, try CRAM_FS=xfs): $out"
else
	ktap_test_fail "large file tier contract failed (rc=$rc): $out"
fi

# 2. no kernel splat during the large-folio round trip (the PMD write fence held)
if [ "$splat" = 0 ]; then
	ktap_test_pass "no KASAN/BUG/UAF/WARN during large file tier round trip"
else
	ktap_test_fail "kernel splat during large file tier round trip (count=$splat)"
fi
