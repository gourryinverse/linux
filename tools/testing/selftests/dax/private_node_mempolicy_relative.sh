...
# cpuset: the public nodes plus any private node that opted into FEAT_USER_NUMA.
...
# node_is_usernuma NID -- true if userspace may place on the node (public nodes
# qualify too: FALLBACK => all features).
node_is_usernuma() {
	nodelist_has "$(cat "$NODE_BASE/has_user_memory" 2>/dev/null)" "$1"
}
