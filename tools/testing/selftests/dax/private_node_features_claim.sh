...
#
# node_features_register() accepts a second range on a node only when it
...
#
#   1. a device declaring an INCONSISTENT mask is refused outright
#   2. the first device's mask becomes the node's mask
#   3. a second device declaring the SAME mask is accepted
#   4. a device declaring a DIFFERENT mask is refused; the mask is unchanged
#   5. once every range is removed the node takes the different mask
#
# Cell 1 runs first and on a still-unclaimed node deliberately: once the node
# carries a mask, a refusal could equally be the different-mask rule, and the
# two would be indistinguishable from here.
#
...
#
# Cell 4 is the discriminating one.  Cell 5 states the contract but does not by
...
# DEMOTION without RECLAIM is rejected: a demotion target needs reclaim as a
# safety valve so it cannot fill permanently.
MASK_BAD=$(( $(pn__feat_mask demotion) | $(pn__feat_mask user_numa) ))
...
# a provider offering this node four devices -- two with the same mask, one with
# another, and one whose mask is inconsistent.
reload_provider() {
	local d
	...
	modprobe dax_test "target_node=$NID" \
		...
		"features=$MASK_A,$MASK_A,$MASK_B,$MASK_BAD" 2>/dev/null
}

# dax devices of this provider, in creation order (mask A, A, B, BAD)
provider_devs() {
	local d
	for d in "$DAX_BASE"/dax*; do
...
[ "$NDEV" = 4 ] ||
	{ ktap_skip_all "dax_test gave $NDEV devices on node $NID, want 4"; exit "$KSFT_SKIP"; }

# shellcheck disable=SC2086
set -- $DEVS
DEV_A1=$1 DEV_A2=$2 DEV_B=$3 DEV_BAD=$4
ktap_print_msg "node $NID: $DEV_A1/$DEV_A2 mask $(printf '%#x' "$MASK_A"), $DEV_B mask $(printf '%#x' "$MASK_B"), $DEV_BAD mask $(printf '%#x' "$MASK_BAD")"
...
# 1. an inconsistent mask is refused before the node is claimed at all
kmem_bind "$DEV_BAD" && bound=1 || bound=0
if [ "$bound" = 0 ]; then
	ktap_test_pass \
		"$DEV_BAD (mask $(printf '%#x' "$MASK_BAD"): DEMOTION without RECLAIM) refused"
else
	ktap_test_fail "$DEV_BAD bound despite naming DEMOTION without RECLAIM"
	echo unplugged > "$DAX_BASE/$DEV_BAD/state" 2>/dev/null
	kmem_unbind "$DEV_BAD"
fi
...
# 3. a second range naming the same mask joins the node
...
# 4. a range naming a different mask is refused, and changes nothing
...
# 5. with every range gone the node is free to take the other mask
