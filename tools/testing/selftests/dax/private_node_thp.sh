...
# (kernel daemon) -- is gated on FEAT_COLLAPSE (N_MEMORY_COLLAPSE).  With the
# feature the node participates in collapse; without it neither may form an
# on-node THP.
# (Placing the base pages via mbind needs FEAT_USER_NUMA.)
#
#   1. MADV_COLLAPSE with FEAT_COLLAPSE: on-node THP forms.
#   2. khugepaged with FEAT_COLLAPSE: on-node THP forms.
#   3. MADV_COLLAPSE WITHOUT FEAT_COLLAPSE: no on-node THP (gate blocks it).
#   4. khugepaged WITHOUT FEAT_COLLAPSE: no on-node THP (gate blocks it).
#
...
# 1. MADV_COLLAPSE is gated on FEAT_COLLAPSE: forms an on-node THP.
out=$("$TOOL" collapse "$PN" 4 2>&1); echo "$out" | sed 's/^/# /'
if [ "$(ahp_of "$out" "$PN")" -gt 0 ] 2>/dev/null; then
	ktap_test_pass "MADV_COLLAPSE formed an on-node THP via FEAT_COLLAPSE"
else
	ktap_test_fail "MADV_COLLAPSE did not form an on-node THP with collapse set ($out)"
fi

# 2. khugepaged collapses private-node folios when FEAT_COLLAPSE is granted.
out=$("$TOOL" khugecollapse "$PN" 4 20 2>&1); echo "$out" | sed 's/^/# /'
if [ "$(ahp_of "$out" "$PN")" -gt 0 ] 2>/dev/null; then
	ktap_test_pass "khugepaged collapsed onto private node $PN via FEAT_COLLAPSE"
else
	ktap_test_fail "khugepaged did not collapse onto private node $PN with collapse set ($out)"
fi

# Re-online WITHOUT FEAT_COLLAPSE (keep user_numa so the base pages still land
...
# 3. MADV_COLLAPSE without FEAT_COLLAPSE: no on-node THP.
out=$("$TOOL" collapse "$PN" 4 2>&1); echo "$out" | sed 's/^/# /'
if [ "$(ahp_of "$out" "$PN")" -eq 0 ] 2>/dev/null; then
	ktap_test_pass "MADV_COLLAPSE formed no on-node THP without FEAT_COLLAPSE (gated)"
else
	ktap_test_fail "MADV_COLLAPSE formed an on-node THP despite collapse cleared ($out)"
fi

# 4. khugepaged without FEAT_COLLAPSE: no on-node THP.
out=$("$TOOL" khugecollapse "$PN" 4 20 2>&1); echo "$out" | sed 's/^/# /'
if [ "$(ahp_of "$out" "$PN")" -eq 0 ] 2>/dev/null; then
	ktap_test_pass "khugepaged formed no on-node THP without FEAT_COLLAPSE (gated)"
else
