...
int main(int argc, char **argv)
{
	...
	fprintf(stderr,
		"diag: present=%d swapped=%d on_cram=%ld pfn=0x%" PRIx64 " read_ok=%d\n",
		present, swapped, nb, pfn_before, read_ok);

	if (nb <= 0) {
		printf("page did not demote to CRAM (on_cram=%ld)\n", nb);
	...
	/* Contract checks. */
	if (!present || swapped)
		rc |= 1;	/* must be present, not swapped */
	if (!read_ok)
		rc |= 2;	/* zero-copy read intact */
	if (!write_ok)
		rc |= 4;	/* write took effect */
	if (!pfn_after || pfn_after == pfn_before)
					rc |= 8;	/* must have promoted (new frame) */
	if (na >= nb)
		rc |= 16;	/* must have left the cram node */
