...
/*
 * Demotion and promotion are counted by core mm, per node, not by CRAM:
 * pgdemote_* and pgpromote_fence{,_failed} in the node's vmstat.  The old
 * cram debugfs counters duplicated these and are gone.
 */
#define NODE_VMSTAT   "/sys/devices/system/node/node%d/vmstat"
...
/* Demotion lands ON the cram node; promotion is counted against its source. */
...
static int t_stress(void)
{
	...
	d0 = cram_demoted();
	p0 = cram_promoted(cram_nid);
	pf0 = cram_promote_failed(cram_nid);
	...
	d1 = cram_demoted();
	p1 = cram_promoted(cram_nid);
	pf1 = cram_promote_failed(cram_nid);
	...
	/*
	 * Pass gates (reliable under concurrency): no livelock and no torn reads
	 * (data integrity).  bad_wr (the
	...
	/*
	 * The demote/promote deltas printed above are diagnostics, not gates.
	 * Neither can be required here:
	 *
	 *  - placement goes through the [TEST] debugfs verb, which calls
	 *    alloc_migration_target() directly rather than running reclaim, so
	 *    it never increments pgdemote_*;
	 *  - an anonymous write promotes via do_wp_page() COW, which is a fresh
	 *    allocation rather than a migration, so it never increments
	 *    pgpromote_fence either.  Only device-nominated promotion does.
	 *
	 * Residency is what the .sh asserts.  This used to claim an advancing
	 * demote count was a pass gate while the check itself was disabled --
	 * the counter it read had been removed from cram debugfs.
	 */
	return 0;
}
