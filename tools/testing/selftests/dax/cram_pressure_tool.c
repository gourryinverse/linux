...
/*
 * CRAM read-only anonymous-memory pressure / livelock battery.
 *
 * Hammers CRAM with simultaneous placement pressure and COW promotion
 ...
 *                  Prints operation and placement counts.
 ...
 * cram device in place, so the pfn check is essential.  The wrapper's dmesg
 * scan covers kernel assertions and other splats.  Residency checks in each
 * operation ensure that a green run cannot be a no-op.
 *
...
static int t_stress(void)
{
	...
	fprintf(stderr,
		"stress: ops=%lu bad=%lu (rd=%lu val=%#x wr=%lu)\n",
		c.ops, c.bad, c.bad_rd, c.bad_val, c.bad_wr);
	 ...
	 * distinguish from an in-place write.  The single-threaded
	 * cram_leak/cram_ops writable-PTE tests cover that contract directly.
	 */
...
int main(int argc, char **argv)
{
	...
	if (!strcmp(sub, "stress"))
		return t_stress();
	if (!strcmp(sub, "promote_race"))
		return t_promote_race();
	if (!strcmp(sub, "drain_race"))
		return t_drain_race();
