 ...
 *
 * Usage: cram_perf_tool <cram_nid> <no-alloc-path> [samples] [mono|tsc]
...
static int cram_nid;
static const char *no_alloc_path;
...
int main(int argc, char **argv)
{
	...
	if (argc < 3) {
		fprintf(stderr,
			"usage: %s <cram_nid> <no-alloc-path> [samples] [mono|tsc]\n",
			argv[0]);
		return 2;
	}
	cram_nid = atoi(argv[1]);
	no_alloc_path = argv[2];
	page_size = sysconf(_SC_PAGESIZE);
	if (argc >= 4 && atol(argv[3]) > 0)
		samples = atol(argv[3]);
	if (argc >= 5 && !strcmp(argv[4], "tsc"))
		clkmode = CLK_TSC;
