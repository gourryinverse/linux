...
#
# This exercises apply_policy_zone() honoring a kernel-zoned private node
# (which is published into N_HIGH_MEMORY like any public node).
#
...
mb=$(fault_mb)
	...
	if [ -z "$D" ]; then
		ktap_test_skip "no device to unplug (boot-provisioned node $PN)"
	elif [ "$after" = unplugged ] && [ "$held" != unplugged ]; then
