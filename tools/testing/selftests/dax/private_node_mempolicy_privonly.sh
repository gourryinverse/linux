...
# A movable-only private node with userland placement: the worst case for the
# worry, and the only shape that actually tests it.
#
# Device-backed, because boot memory cannot be re-zoned.  pn_online_as() accepts
# "online" on a boot node and returns success without doing anything, leaving it
# ZONE_NORMAL -- where L3's page tables land on the node legitimately and the
# assertion is measuring nothing.  Asking for online_movable makes that case
# skip instead.
pn_require_dax
pn_set user_numa 1
...
# Fault more than the private node can hold; with no RECLAIM feature the bound
# task must OOM (CONSTRAINT_MEMORY_POLICY) rather than refault forever.
