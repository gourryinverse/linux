...
#
# Private-node feature-claim lifetime across a driver unbind/rebind.
#
# A node's feature mask (claimed by node_features_register()) must be
# dropped when the node's *last memory is removed*, not merely when it is
# offlined.  The driver unbind path
#
#   dev_dax_kmem_remove -> dax_kmem_remove_ranges -> remove_memory()
#     -> try_remove_memory() -> try_offline_node()
#
# removes the memory of an already-offline device.  A claim left standing on a
# memoryless node would refuse every later claim naming a different mask; that
# refusal is what private_node_features_claim.sh asserts directly.  This test
# covers the surrounding lifecycle: the device comes back and the node returns
# as a private node with its resources intact.
#
# Note offlining alone (memoryN/state) only offlines the private node; it never
# calls node_features_unregister(), so the claim legitimately persists
# across an offline/online cycle.  Its lifetime is only exercised by
# add/remove, which is why this test drives a full unbind (remove) and rebind.
#
...
up_private() {
	...
	pn_set private 1
	pn_set user_numa 1
	# online_movable, not online: both callers below assert state ==
	# online_movable, and "online" is auto -- it follows
	# memhp_default_online_type, which lands in ZONE_NORMAL here.
...
# and the node goes memoryless -- the point at which the claim must be dropped.
echo "$DAX" > "$DRV/unbind" 2>/dev/null
leaked=$(grep -cE " : ${DAX}\$" /proc/iomem)
priv_after_unbind=$(pn_is_private && echo 1 || echo 0)

# Rebind and bring the node back up as private, re-claiming the same mask.
...
if [ "$leaked" = 0 ] && [ "$rc" = 0 ] && [ "$state" = online_movable ] && \
   [ "$back_private" = 1 ]; then
	ktap_test_pass "private node survives unbind/rebind (claim dropped at remove; re-onlines as a private node)"
else
	ktap_test_fail "unbind/rebind lost the private node (iomem_leaked=$leaked online_rc=$rc state=$state back_private=$back_private)"
fi
