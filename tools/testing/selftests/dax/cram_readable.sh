...
#
# CRAM read-only tier round-trip test (driverless).
#
...
# 1. read-only tier round trip: demote present-RO on cram, zero-copy read, COW-promote
if [ "$rc" = 0 ]; then
	ktap_test_pass "read-only tier: demote present-RO on cram $PN, read zero-copy, write promoted"
elif [ "$rc" = 3 ]; then
	ktap_test_skip "anon did not demote to CRAM (environmental): $out"
else
	ktap_test_fail "read-only tier contract failed (rc=$rc): $out"
fi

# 2. no kernel splat during the round trip
if [ "$splat" = 0 ]; then
	ktap_test_pass "no KASAN/BUG/UAF/WARN during read-only tier round trip"
else
	ktap_test_fail "kernel splat during read-only tier round trip (count=$splat)"
fi
