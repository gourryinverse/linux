...
# task may place memory there, the USER_NUMA feature gates whether userland
# numactl ops are allowed at all, and the zonelist keeps GENERAL allocations
# off private nodes regardless of membership.  These are three independent
# constraints.
#
...
#      membership.
#  G9) offlining a driver-owned node holding a node-bound anon mapping migrates its pages
...
if [ -z "$D" ]; then
	...
	if [ "${onP0:-0}" -le 0 ] 2>/dev/null || ! nodelist_has "${pol0#*:}" "$PN"; then
		...
		ktap_test_skip "G9 node-bound anon mapping not established (on$PN=$onP0 pol='$pol0'); cannot probe scrub"
	else
