...
#   CRAM_FS_CELL_TIMEOUT  per-cell budget in seconds (default 300).  A debug
#                 kernel -- lockdep, DEBUG_VM, PAGE_TABLE_CHECK -- runs these
#                 several times slower, and a cell killed by the timeout reads
#                 as a teardown wedge rather than as "too slow".
#
...
for fs in $FS_LIST; do
	for t in $FILE_TESTS; do
		out=$(CRAM_FS="$fs" CRAM_FS_DISK="$DISK" timeout "${CRAM_FS_CELL_TIMEOUT:-300}" bash "$DIR/$t" 2>&1)
		rc=$?
		# A cell can print a complete KTAP verdict and then hang in its own
		# EXIT trap unplugging the node, so timeout kills it and rc is 124.
		# That rc describes the teardown, not the test: judge by the verdict
		# whenever the cell produced one.  Checked after the skip case, since
		# a skip also emits a Totals line.
		tot=$(printf '%s\n' "$out" | grep -m1 -aoE '# Totals:.*')
		# ' fail:N' with the leading space, or xfail: also matches.
		nfail=$(printf '%s\n' "$tot" | grep -oE ' fail:[0-9]+' | grep -oE '[0-9]+' | head -1)
		if [ "$rc" = 0 ]; then
			ktap_test_pass "$fs/$t"
		...
		elif [ -n "$tot" ] && [ "${nfail:-1}" = 0 ]; then
			ktap_test_pass "$fs/$t (rc=$rc, teardown wedged) [$tot]"
		else
