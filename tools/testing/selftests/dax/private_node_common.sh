...
#   - set "state=unplugged"
#   - set the device's debugfs "mm_capabilities" to a value other than
#     NODE_MEMORY_CAP_ALL (~0); the set bits are the services
#     (reclaim/user_numa/demotion/...) it opts into
#   - set "state=online"
#
# mm_capabilities == NODE_MEMORY_CAP_ALL brings the memory up as a normal
# N_MEMORY node instead.  It lives in debugfs rather than sysfs because the
# mask is a raw in-kernel bit layout; dax_file and adistance are ordinary
# sysfs attributes.
#
...
DAX_DEBUGFS=/sys/kernel/debug/dax_kmem

# pn_need_debugfs -- mount debugfs if needed; SKIP if the capability knob is absent.
pn_need_debugfs() {
	grep -q debugfs /proc/mounts || mount -t debugfs none /sys/kernel/debug 2>/dev/null
	[ -d "$DAX_DEBUGFS" ] ||
		{ ktap_skip_all "no $DAX_DEBUGFS (need CONFIG_DEBUG_FS=y and dax_kmem loaded)"; exit "$KSFT_SKIP"; }
}

# pn_capfile DEV -- path to DEV's capability knob.
pn_capfile() {
	echo "$DAX_DEBUGFS/$1/mm_capabilities"
}
...
pn_dev_getcap() {
	...
	[ $(( $(cat "$(pn_capfile "$1")" 2>/dev/null) & bit )) -ne 0 ] &&
		echo 1 || echo 0
}
...
pn__make_private() {
	...
	echo 0 > "$(pn_capfile "$(basename "$d")")" 2>/dev/null
...
pn_provision() {
	...
	pn_need_debugfs	# the capability knob lives there, and is read below
	...
	CAPF=$(pn_capfile "$DAX")
	{ [ -e "$D/state" ] && [ -e "$CAPF" ]; } ||
		{ ktap_skip_all "$DAX has no capability knob at $CAPF"; exit "$KSFT_SKIP"; }
}
...
pn_set() {
	...
	private)	# base config: 1 => private (no caps), 0 => normal (all caps)
		[ "$2" = 1 ] && echo 0 > "$CAPF" 2>/dev/null \
			      || echo "$PN_CAP_ALL" > "$CAPF" 2>/dev/null ;;
...
# private node: onlineness shows in has_memory, and it is absent from
# has_fallback_memory
...
pn_reset() {
	...
	[ -e "$CAPF" ] || return 0
	pn_dev_is_private "$DAX" && echo 0 > "$CAPF" 2>/dev/null
	return 0
}
...
pn_provision_all() {
	modprobe -q nd_e820 dax_pmem device_dax nd_pmem 2>/dev/null
	modprobe -q kmem 2>/dev/null
	pn_need_debugfs	# the capability knob lives there
	if command -v ndctl >/dev/null 2>&1; then
		local r
		for r in $(ndctl list -R 2>/dev/null | grep -oE 'region[0-9]+'); do
			ndctl create-namespace -m devdax -e "${r/region/namespace}.0" -f \
...
# includes private nodes, so the discriminator is has_fallback_memory: the nodes
# an allocation reaches without naming them.
pn_node_is_public() {
	nodelist_has "$(cat "$NODE_BASE/has_fallback_memory" 2>/dev/null)" "$1"
}
