...
# pn_modprobe MOD... -- load each module.  modprobe takes ONE module name and
# treats the rest as its PARAMETERS, so a list has to be a loop.
pn_modprobe() {
	local m
	for m in "$@"; do modprobe -q "$m" 2>/dev/null; done
}
...
pn_provider_load() {
	...
	pn_modprobe nd_e820 dax_pmem nd_pmem device_dax kmem
	...
	modprobe -q dax_test \
		...
		"features=${PN_DAX_TEST_FEATURES:-0x186}" \
...
pn_provision() {
	...
	if [ -z "$nid" ]; then
		...
		if [ -z "${PN_DAX_TEST_RANGE_START:-}" ]; then
			pn_modprobe nd_e820 dax_pmem device_dax nd_pmem kmem
			if command -v ndctl >/dev/null 2>&1; then
				local r
...
pn__feat_mask() {
	case "$1" in
	public) echo 1;;         reclaim) echo 2;;
	demotion) echo 4;;       user_numa) echo 128;;
	user_write) echo 256;;
	*) echo 0;;
	esac
}
...
# pn__candidates -- nodes pn__reselect() may choose from, best first.
#
# Online private nodes, then -- only for a test that asked for a device --
# memoryless nodes whose dax device could bring one up.  The second group
# exists because pn_reset() unplugs the device to reach a clean baseline, which
# takes the node out of has_memory and therefore out of pn_private_nodes().  A
# hotplug test would otherwise drop its own node the instant it reached its
# baseline, and every later pn_set would skip for want of a node sitting right
# there, unplugged.  pn_node_features() still resolves for these: it reads the
# provisioning source, not the node's current state.
pn__candidates() {
	local d nid

	pn_private_nodes
	[ "$PN_NEED_DAX" = 1 ] || return 0

	# Enumerated from the dax devices, not from $NODE_BASE/node*: unplugging
	# the last of a node's memory takes its sysfs directory with it, so the
	# node we are looking for is exactly the one that has stopped existing
	# there.  The device outlives it and still names it.
	for d in "$DAX_BASE"/dax*; do
		[ -e "$d/target_node" ] && [ -e "$d/state" ] || continue
		nid=$(cat "$d/target_node" 2>/dev/null)
		[ "$nid" -ge 0 ] 2>/dev/null || continue
		pn_nodelist_has "$NODE_BASE/has_memory" "$nid" && continue
		pn_nodelist_has "$NODE_BASE/has_public_memory" "$nid" && continue
		echo "$nid"
	done
}
...
pn_set() {
	...
	case "$1" in
	reclaim|demotion|user_numa|user_write)
		bit=$(pn__feat_mask "$1")
		if [ "$2" = 1 ]; then
