...
/* NODE_MEMORY_CAP_USER_NUMA: the node accepts userspace-directed placement. */
#define NODE_MEMORY_CAP_USER_NUMA	(1UL << 8)

/*
 * True if a bind naming @node can succeed: it must hold memory, and a private
 * node must have opted into userspace placement.  A kernel without per-node
 * capabilities has no mem_features and every node is bindable.
 */
static int node_is_bindable(int node)
{
	...
	snprintf(path, sizeof(path),
		 "/sys/devices/system/node/node%d/mem_features", node);
	f = fopen(path, "r");
	if (!f)
		return 1;
	n = fscanf(f, "%li", &feat);
	...
	return n != 1 || (feat & NODE_MEMORY_CAP_USER_NUMA);
}
