...
/* Membership in a "0,2-3" style node-state nodelist; -1 if it is absent. */
static int node_in_nodelist(const char *path, int node)
{
...
/*
 * True if a bind naming @node can succeed: it must hold memory, and a private
 * node must have opted into userspace placement.  has_user_memory is the
 * node-state nodelist for that opt-in; a kernel without private memory nodes
 * does not publish it, and there every memory node is bindable.
 */
static int node_is_bindable(int node)
{
	int in_list;
	...
	in_list = node_in_nodelist("/sys/devices/system/node/has_user_memory",
				   node);
	return in_list < 0 ? 1 : in_list;
}
...
int main(int argc, char **argv)
{
	...
	if (gmem < 0) {
		 ...
		 * rejected at create with EINVAL (the bind policy fails), not
		 * produce an opaque failure later.
		 */
		if (!node_is_bindable(target_node) && errno == EINVAL) {
	...
	if (!node_is_bindable(target_node))
