...
static ssize_t private_node_show(struct device *dev,
			     struct device_attribute *attr, char *buf)
{
	struct dev_dax *dev_dax = to_dev_dax(dev);

	return sysfs_emit(buf, "%d\n", dev_dax->private_node);
}

static ssize_t private_node_store(struct device *dev,
			      struct device_attribute *attr,
			      const char *buf, size_t len)
{
	struct dev_dax *dev_dax = to_dev_dax(dev);
	...
	if (dev_dax->private_node != val && dev->driver &&
	    to_dax_drv(dev->driver)->type == DAXDRV_KMEM_TYPE) {
		up_write(&dax_dev_rwsem);
		return -EBUSY;
	}

	dev_dax->private_node = val;
	up_write(&dax_dev_rwsem);

	return len;
}
static DEVICE_ATTR_RW(private_node);
...
static struct dev_dax *__devm_create_dev_dax(struct dev_dax_data *data)
{
	...
	dev_dax->private_node = data->private_node;
