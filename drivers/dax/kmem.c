 ...
 */
static ssize_t mm_capabilities_show(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	struct dax_kmem_data *data = dev_get_drvdata(dev);
	...
	if (!data)
		return -ENXIO;
	return sysfs_emit(buf, "0x%llx\n", data->mm_capabilities);
}

static ssize_t mm_capabilities_store(struct device *dev,
				     ...
				     const char *buf, size_t len)
{
	...
	if ((caps & NODE_MEMORY_CAP_FALLBACK) && !(caps & NODE_MEMORY_CAP_USER_NUMA))
		return -EINVAL;

	data->mm_capabilities = caps;
	return len;
}
static DEVICE_ATTR_RW(mm_capabilities);
