...
struct cxl_pcpu_flush {
	...
	spinlock_t swap_lock;			/* protects active swap */
};
...
static int cxl_flush_kthread_fn(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		for_each_possible_cpu(cpu) {
			...
			spin_lock(&pcpu->swap_lock);
			dirty = rcu_replace_pointer(pcpu->active, spare, true);
			spin_unlock(&pcpu->swap_lock);
...
static void cxl_flush_overflow_work(struct work_struct *work)
{
	...
	spin_lock_irqsave(&pcpu->swap_lock, flags);
	dirty = rcu_replace_pointer(pcpu->active, spare, true);
	spin_unlock_irqrestore(&pcpu->swap_lock, flags);
...
static void cxl_compression_pre_teardown(void *data)
{
	 ...
	 * CRAM callbacks require the node_device to still
...
static void cxl_compression_post_teardown(void *data)
{
	...
	/* Release sysram device reference from cxl_region_find_sysram() */
	if (tctx->sysram)
		put_device(&tctx->sysram->dev);
}
...
static int convert_dvsec_to_sysram(struct range *hpa, struct pci_dev *pdev)
{
	...
	if (!flush_ctx->pcpu) {
		put_device(sdev);
		flush_record_free(flush_ctx->flush_record,
				  flush_ctx->flush_record_pages);
		return -ENOMEM;
	}
...
static void cxl_compression_remove(struct pci_dev *pdev)
{
	 ...
	 *   2. post_teardown: kthread stop, flush cleanup
	 *   3. sysram_unregister: device_unregister (sysram->res is NULL
