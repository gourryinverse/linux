 ...
 *
 * Minimal driver for CXL memory devices that registers memory as a
 * private node with mempolicy support but no PTE controls.  The
...
#define CXL_MEMPOLICY_FLAGS (PGMAP_OPS_MIGRATION | PGMAP_OPS_MEMPOLICY)
			      ...
			      struct cxl_region *cxlr)
{
	...
	/*
	 * Set per-node callbacks on the node_device registered by
	 * the hotplug path (add_private_memory_driver_managed).
	 */
	rcu_read_lock();
	{
		...
		if (!nd) {
			rcu_read_unlock();
			dev_err(cxl_region_dev(cxlr),
				"no node_device on node %d\n", nid);
			return -ENODEV;
		}
		nd->migrate_to = cxl_mempolicy_migrate_to;
		nd->flags |= CXL_MEMPOLICY_FLAGS;
	}
	rcu_read_unlock();

	node_set(nid, ctx->nids);
	dev_info(&cxlmd->dev,
...
static int cxl_mempolicy_attach_dvsec(struct cxl_memdev *cxlmd)
{
	...
	for (i = 0; i < info->ranges; i++) {
		...
		rcu_read_lock();
		{
			struct node_device *nd = rcu_dereference(
				NODE_DATA(nid)->node_dev);
...
static int cxl_mempolicy_attach_probe(struct cxl_memdev *cxlmd)
{
	...
	if (nr > ARRAY_SIZE(regions))
		nr = ARRAY_SIZE(regions);
	for (i = 0; i < nr; i++) {
...
static void cxl_mempolicy_remove(struct pci_dev *pdev)
{
...

	{
		...
		for_each_node_mask(nid, ctx->nids) {
			rcu_read_lock();
			{
				struct node_device *nd = rcu_dereference(
					NODE_DATA(nid)->node_dev);
