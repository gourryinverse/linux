...
int node_private_register(int nid, struct node_private *np)
{
	...
	pgdat = NODE_DATA(nid);
	p = rcu_dereference_protected(pgdat->private,
				      lockdep_is_held(&node_private_lock));

	if (node_state(nid, N_MEMORY) && !p) {
		ret = -EBUSY;
		goto out;
	}
	...
	/* Re-registration with the same np is a no-op */
	if (p == np)
		goto out;
	...
	rcu_assign_pointer(pgdat->private, np);
out:
 ...
 * has been offlined and the node is no longer in N_MEMORY.
 * When successful, drops the refcount to 0 indicating the driver can
 * free its context.
 *
 * N_MEMORY state is cleared by offline_pages() when the last
...
int node_private_unregister(int nid)
{
	...
	pgdat = NODE_DATA(nid);
	np = rcu_dereference_protected(pgdat->private,
				       lockdep_is_held(&node_private_lock));
	if (!np) {
		mutex_unlock(&node_private_lock);
		return 0;
	...
	rcu_assign_pointer(pgdat->private, NULL);
	mutex_unlock(&node_private_lock);
...
/* has_memory includes private nodes that support mempolicy. */
