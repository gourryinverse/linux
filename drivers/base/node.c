...
static struct node_attr node_state_attr[] = {
	...
	[N_MEMORY_COMMON] = _NODE_ATTR(has_common_memory, N_MEMORY_COMMON),
...
static struct attribute *node_state_attrs[] = {
	...
	&node_state_attr[N_MEMORY_COMMON].attr.attr,
...
/**
 * node_features_register - claim the mm features of a node's memory
 * @nid: the node
 * @features: NODE_MEMORY_FEAT_* services the memory opts into;
 *            %NODE_MEMORY_FEAT_ALL is an ordinary common node
 *
 * A node whose mask lacks NODE_MEMORY_FEAT_COMMON is private: kept off the
 ...
 */
int node_features_register(int nid, unsigned long features)
{
	...
	if (features & NODE_MEMORY_FEAT_COMMON &&
		...
		return -EINVAL;
	if (features != NODE_MEMORY_FEAT_ALL &&
...
/**
 * node_features_unregister - return a node to the common default
 * @nid: the node
 *
 * Safe on a common node. The caller must hold the memory hotplug
 * lock and must have offlined the node's memory first.
 */
void node_features_unregister(int nid)
{
	if ((unsigned int)nid >= MAX_NUMNODES || !node_possible(nid))
		return;
...
EXPORT_SYMBOL_GPL(node_features_unregister);
