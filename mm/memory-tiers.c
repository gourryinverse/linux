...
int next_demotion_node(int node, const nodemask_t *allowed_mask)
{
	...
	return find_next_best_node_in(node, &mask, &node_states[N_MEMORY_DEMOTION]);
}
...
static void disable_all_demotion_targets(void)
{
	...
	for_each_node_state(node, N_MEMORY_DEMOTION) {
...
static void establish_demotion_targets(void)
{
	...
	for_each_node_state(node, N_MEMORY_DEMOTION) {
		 ...
		 * memtier nodelist.
		 */
		nodes_andnot(tier_nodes, node_states[N_MEMORY_DEMOTION], tier_nodes);
	 ...
	 * preferred node.
	 */
	lower_tier = node_states[N_MEMORY_DEMOTION];
	list_for_each_entry(memtier, &memory_tiers, list) {
		/*
...
static struct memory_tier *set_node_memory_tier(int node)
{
	...
	if (!node_state(node, N_MEMORY_DEMOTION))
		return ERR_PTR(-EINVAL);
...
static int __init memory_tier_late_init(void)
{
	...
	/* Assign each uninitialized N_MEMORY_DEMOTION node to a memory tier. */
	for_each_node_state(nid, N_MEMORY_DEMOTION) {
		/*
