...
static void establish_demotion_targets(void)
{
	 ...
	 * Private nodes are already in N_MEMORY; remove those without
	 * PGMAP_OPS_DEMOTION so they don't appear in demotion targets.
	 */
	all_memory = node_states[N_MEMORY];
	for_each_node_state(node, N_MEMORY) {
		if (node_is_private(node) &&
		    !node_device_has_flag(node, PGMAP_OPS_DEMOTION))
			node_clear(node, all_memory);
	}
...
void memory_tier_refresh_demotion(void)
{
	 ...
	 */
	for_each_node_state(nid, N_MEMORY) {
		if (node_is_private(nid) && !__node_get_memory_tier(nid))
