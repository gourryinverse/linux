 ...
 *
 * Manages folios demoted to private nodes via the standard kernel
...
static struct folio *alloc_cram_folio(struct folio *src, unsigned long private)
{
	...
	if (node_device_alloc_blocked(nid))
		return NULL;
...
static const struct dev_pagemap_ops cram_pgmap_ops = {
	.free_node_folio	= cram_free_folio_cb,
	.folio_migrate		= cram_folio_migrate,
	.handle_fault		= cram_handle_fault,
};

#define CRAM_NODE_FLAGS (PGMAP_OPS_MIGRATION | PGMAP_OPS_DEMOTION | \
			 PGMAP_OPS_NUMA_BALANCING | PGMAP_OPS_PROTECT_WRITE | \
			 PGMAP_OPS_RECLAIM)
			       ...
			       cram_flush_cb_t flush_cb, void *flush_data)
{
	...
	/*
	 * Set per-node callbacks on the already-registered node_device.
	 * The node_device was created by the hotplug path
	 * (add_private_memory_driver_managed → node_device_register).
	 */
	rcu_read_lock();
	{
		struct node_device *nd = rcu_dereference(NODE_DATA(nid)->node_dev);
		...
		nd->migrate_to = cram_migrate_to;
		nd->reclaim_policy = cram_reclaim_policy;
		nd->flags |= CRAM_NODE_FLAGS;
	}
	rcu_read_unlock();
...
int cram_unregister_private_node(int nid)
{
	...
	/* Clear per-node callbacks on the node_device */
	rcu_read_lock();
	{
		struct node_device *nd = rcu_dereference(NODE_DATA(nid)->node_dev);
...
void cram_set_pressure(int nid, unsigned int pressure)
{
	...
	/* Block allocations at maximum pressure */
	if (nd)
		WRITE_ONCE(nd->alloc_blocked,
			   pressure >= CRAM_PRESSURE_MAX);
	rcu_read_unlock();
