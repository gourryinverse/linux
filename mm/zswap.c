...
static struct page *zswap_compress_direct(struct page *src,
					  struct zswap_entry *entry)
{
	int nid = first_node(node_states[N_PRIVATE]);
	struct page *dst;
	gfp_t gfp;

	/* FIXME: Query Node Type = Compressed RAM */
	if (nid == NUMA_NO_NODE)
		return NULL;

	gfp = GFP_NOWAIT | __GFP_NORETRY | __GFP_HIGHMEM | __GFP_MOVABLE |
	      __GFP_THISNODE;
	dst = __alloc_pages(gfp, 0, nid, &node_states[N_PRIVATE]);
	if (!dst)
		return NULL;
			   ...
			   struct zswap_pool *pool)
{
	...
	/* HACK: Try to shunt directly to compressed ram */
