				 ...
				 enum mmop online_type, struct node_private *np)
{
	...
	return ret;
error:
	/* If the node ended up with no memory, undo the private registration. */
 ...
 * @np: private-node descriptor selecting the node's capabilities, or %NULL to
 *      add the memory as an ordinary public node
 *
...
EXPORT_SYMBOL_FOR_MODULES(add_private_memory_driver_managed, "kmem");
