		       ...
		       struct zone *zone, struct memory_group *group)
{
	...
	if (node_arg.nid >= 0) {
		if (rcu_access_pointer(NODE_DATA(nid)->node_dev))
				 ...
				 enum mmop online_type, void *np)
{
	...
	if (np) {
		ret = node_device_register(nid, np);
		if (ret)
			return ret;
 ...
 * @np: Optional node_device for managed device nodes (or NULL)
 *
