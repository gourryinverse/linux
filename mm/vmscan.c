...
static inline bool zone_reclaim_allowed(struct zone *zone, gfp_t gfp_mask)
{
	if (node_is_private(zone_to_nid(zone)))
		return zone_device_has_flag(zone, PGMAP_OPS_RECLAIM);
		       ...
		       struct mem_cgroup *memcg)
{
	...
	if (node_device_alloc_blocked(demotion_nid))
		return false;
				     ...
				     struct pglist_data *pgdat)
{
	...
	while (node_is_private(target_nid)) {
		...
		ret = node_device_migrate_to(demote_folios, target_nid,
	...
	return nr_succeeded + nr_private;
}
...
static int balance_pgdat(pg_data_t *pgdat, int order, int highest_zoneidx)
{
	...
	node_device_reclaim_policy(pgdat->node_id, &policy);
