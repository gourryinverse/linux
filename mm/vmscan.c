...
static void *lru_gen_seq_next(struct seq_file *m, void *v, loff_t *pos)
{
	...
	nid = next_node(nid, node_states[N_MEMORY_RECLAIM]);
	if (nid == MAX_NUMNODES) {
		memcg = mem_cgroup_iter(NULL, memcg, NULL);
		if (!memcg)
			return NULL;

		nid = first_node(node_states[N_MEMORY_RECLAIM]);
	}
...
static int lru_gen_seq_show(struct seq_file *m, void *v)
{
	...
	if (nid == first_node(node_states[N_MEMORY_RECLAIM])) {
...
static void shrink_node(pg_data_t *pgdat, struct scan_control *sc)
{
	...
	/* Skip nodes that do not permit generic reclaim. */
