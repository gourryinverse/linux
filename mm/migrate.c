		...
		struct vm_area_struct *vma, unsigned long addr, void *arg)
{
	...
	while (page_vma_mapped_walk(&pvmw)) {
		...
		pte = folio_managed_fixup_migration_pte(new, pte, old_pte);
...
int migrate_folios_to_node(struct list_head *folios, int nid,
			   ...
			   enum migrate_reason reason)
{
	if (node_device_has_flag(nid, PGMAP_OPS_MIGRATION))
		return node_device_migrate_to(folios, nid, mode,
					      reason, NULL);
				...
				const void __user **pages, int *status)
{
	...
	for (i = 0; i < nr_pages; i++) {
		...
		if (folio) {
			if (is_zero_folio(folio) || is_huge_zero_folio(folio))
				err = -EFAULT;
			else if (unlikely(folio_is_device_managed(folio)))
