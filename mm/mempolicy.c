		     ...
		     const nodemask_t *nodes, struct nodemask_scratch *nsc)
{
	 ...
	 *
	 * If F_PRIVATE is set, allow !N_MEMORY_COMMON && N_MEMORY_USER_NUMA.
	 * If this isn't a BIND policy, only N_MEMORY_COMMON nodes are eligible.
	 *
	 * If a private node is USER_NUMA eligible, it can be remapped.
	 */
	...
	if (pol->mode != MPOL_BIND)
		nodes_and(nsc->mask1, nsc->mask1, node_states[N_MEMORY_COMMON]);
	...
	if (!nodes_subset(nsc->mask2, node_states[N_MEMORY_COMMON]))
...
static void mpol_rebind_nodemask(struct mempolicy *pol, const nodemask_t *nodes)
{
	...
	/* Never introduce USER_NUMA-ineligible nodes through a rebind. */
	...
	/* An empty nodemask resets to the cpuset's common memory nodes. */
	if (nodes_empty(tmp) && nodes_empty(keep) && had_user)
		nodes_and(tmp, *nodes, node_states[N_MEMORY_COMMON]);
	/* Retain driver-bound private nodes that remain in the cpuset. */
	nodes_or(pol->nodes, tmp, keep);

	/* Only MPOL_BIND has the nodemask needed to confine private memory. */
	if (pol->mode != MPOL_BIND)
		nodes_and(pol->nodes, pol->nodes, node_states[N_MEMORY_COMMON]);

	/* MPOL_F_PRIVATE exactly tracks private nodes in the binding. */
	if (!nodes_subset(pol->nodes, node_states[N_MEMORY_COMMON]))
		pol->flags |= MPOL_F_PRIVATE;
	else
		pol->flags &= ~MPOL_F_PRIVATE;
}
			...
			unsigned long end, struct mm_walk *walk)
{
	...
	for (; addr != end; pte += nr, addr += nr * PAGE_SIZE) {
		...
		if (!folio || !folio_allows_user_numa(folio))
			continue;
		...
		bool is_private_single_threaded)
{
	...
	if (!folio || !folio_is_common_memory(folio) || folio_test_ksm(folio))
		return false;
		     ...
		     const nodemask_t *to, int flags)
{
	long nr_failed = 0;
	long err = 0;
	nodemask_t tmp, from_user, to_user;

	nodes_and(from_user, *from, node_states[N_MEMORY_USER_NUMA]);
	nodes_and(to_user, *to, node_states[N_MEMORY_USER_NUMA]);
	from = &from_user;
	to = &to_user;
				...
				const unsigned long __user *new_nodes)
{
	...
	/* All destination nodes must support userspace NUMA placement. */
				   ...
				   pgoff_t ilx, int *nid, unsigned int *alloc_flags)
{
	...
	/* Select the private zonelist if the policy warrants it */
	if (alloc_flags)
	...
	case MPOL_BIND:
		 ...
		 * If the nodemask would be forcibly relaxed by policy, do not
		 * allow ALLOC_ZONELIST_PRIVATE, as this may cause an allocation
		 * to land on an unsupported private node.
		 */
