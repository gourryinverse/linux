		     ...
		     const nodemask_t *nodes, struct nodemask_scratch *nsc)
{
	...
	/* If any private node is still present, set MPOL_F_PRIVATE */
				   ...
				   pgoff_t ilx, int *nid, unsigned int *alloc_flags)
{
	...
	case MPOL_BIND:
		 ...
		 * A bind that does not intersect the cpuset is relaxed drops
		 * the nodemask and reverts to the public fallback zonelist.
		 */
		if (apply_policy_zone(pol, gfp_zone(gfp))) {
			if (cpuset_nodemask_valid_mems_allowed(&pol->nodes))
				nodemask = &pol->nodes;
			else if (alloc_flags)
