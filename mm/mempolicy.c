...
static long do_get_policy_node(struct mempolicy *pol)
{
	 ...
	 * node that will be selected for interleave
	 */
...
{
	unsigned short legacy_mode = (*mode & ~MPOL_MODE_FLAGS);

	*flags = *mode & MPOL_MODE_FLAGS;
	*mode = legacy_mode;

	return validate_mpol_flags(legacy_mode, flags);
}
		...
		unsigned long, flags)
{
	...
	/* if home node give, validate it is online */
	if (kargs.home_node >= 0) {
				...
				unsigned long flags)
{
	 ...
	 * get_mempolicy2 interfaces and the arg stucture made extensible
	 */
