...
/*
 * Managed device node zones require PGMAP_OPS_COMPACTION to opt in.
 * Normal zones always support compaction.
 */
static inline bool zone_supports_compaction(struct zone *zone)
{
	...
	if (!node_is_private(zone_to_nid(zone)))
		return true;
	return zone_device_has_flag(zone, PGMAP_OPS_COMPACTION);
#else
	return true;
#endif
}
