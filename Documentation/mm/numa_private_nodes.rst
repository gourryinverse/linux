...
* ``/sys/devices/system/node/nodeN/mem_features`` -- a hex bitmask of the node's
  ``NODE_MEMORY_CAP_*`` capabilities; the absence of the fallback bit marks the
  node private
...
The ``dax_kmem`` driver (``drivers/dax/kmem.c``) exposes an ``mm_capabilities``
attribute (a ``NODE_MEMORY_CAP_*`` mask) that selects how the DAX device's
memory is hotplugged.  ``NODE_MEMORY_CAP_ALL`` (``~0``, the default) brings it up
as an ordinary N_MEMORY node; any other value brings it up as a private node
whose set bits are the only services it opts into.  It is writable only while the
device is unplugged.
