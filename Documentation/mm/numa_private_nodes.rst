...
.. list-table::
   ...
   * - Can an allocation that named no node land here?
     - ``N_MEMORY_PUBLIC``
   * - May reclaim or demotion operate on the node?
     - ``N_MEMORY_RECLAIM`` or ``N_MEMORY_DEMOTION``
...
THP collapse, DAMON and automatic NUMA balancing.  Those services treat a
private-node folio like a ``ZONE_DEVICE`` folio and have no private-node opt-in.
